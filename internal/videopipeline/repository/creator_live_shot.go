...
func (p *Postgres) ConfirmCreatorLiveShotPlan(
	...
	return withSerializable(ctx, p.pool, func(tx pgx.Tx) (controlplane.Stored[controlplane.CreatorLiveShotRun], error) {
		...
		if replayed {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{Value: replay, Replayed: true}, nil
		}
		...
		if err := tx.QueryRow(ctx, `
			...
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, fmt.Errorf("lock creator live-shot plan: %w", err)
		}
		if actorID != command.Actor.ActorID {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, controlplane.NewNotFoundError("creator live-shot plan", planIDRaw)
		}
		if !command.LiveCallsEnabled {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, controlplane.NewPolicyError(controlplane.CodeLiveCallsDisabled, "live Provider calls are not armed", "set VIDEO_LIVE_CALLS_ENABLED=true only in the authorized runtime")
		}
		if !command.Confirmed {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, controlplane.NewPolicyError(controlplane.CodeValidation, "confirmed must be true", "explicitly confirm the exact plan")
		}
		if command.PlanHash != planHash {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, controlplane.NewConflictError(controlplane.CodePlanHashMismatch, "planHash differs from the immutable plan")
		}
		if !now.Before(expiresAt) {
			_, _ = tx.Exec(ctx, `UPDATE video_pipeline.creator_live_shot_plans SET state='EXPIRED' WHERE id=$1 AND state='READY'`, planID)
		...
		if state != "READY" {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, controlplane.NewConflictError(controlplane.CodePlanStale, "the live-shot plan is no longer confirmable")
		}
		...
		if !sameCreatorCapability(storedCapability, command.Route) || route.CapabilityHash != command.Route.SnapshotHash {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, controlplane.NewConflictError(controlplane.CodePlanStale, "the live Adapter capability changed after planning")
		}
		...
		if _, err := tx.Exec(ctx, `SELECT pg_advisory_xact_lock(hashtextextended($1, 0))`, "creator-project:"+actorID); err != nil {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, fmt.Errorf("lock creator project quota: %w", err)
		}
		var usedTasks int
		var usedTokens int64
		var active int
		if err := tx.QueryRow(ctx, `
			...
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, fmt.Errorf("read creator project quota: %w", err)
		}
		if usedTasks != plannedTasks || usedTokens != plannedTokens || active != plannedActiveRuns {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, controlplane.NewConflictError(controlplane.CodePlanStale, "the project budget or concurrency snapshot changed after planning")
		}
		if active >= 1 {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, controlplane.NewConflictError(controlplane.CodeConcurrencyLimit, "one live shot is already active for this project")
		}
		if usedTasks+1 > creatorProjectTaskLimit || usedTokens+creatorVideoTokenLimit > creatorProjectVideoTokenLimit {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, controlplane.NewPolicyError(controlplane.CodeProjectBudgetExceeded, "the project subscription task/token allowance is exhausted", "start a new project allowance before confirming another shot")
		}
		...
		if err := tx.QueryRow(ctx, `
			...
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, fmt.Errorf("verify creator approval bindings: %w", err)
		}
		if !bindingsValid {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, controlplane.NewConflictError(controlplane.CodePlanStale, "rights, safety, or budget approval no longer matches the plan")
		}
		...
		if err != nil {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, fmt.Errorf("bind creator token reservation: %w", err)
		}
		...
		if _, err := tx.Exec(ctx, `
			...
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, fmt.Errorf("insert creator confirmation operation: %w", err)
		}
		_, err = tx.Exec(ctx, `
		...
		if err != nil {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, fmt.Errorf("insert creator provider intent: %w", err)
		}
		if _, err := tx.Exec(ctx, `UPDATE video_pipeline.creator_live_shot_plans SET state='CONFIRMED', confirmed_at=$2 WHERE id=$1`, planID, now); err != nil {
			return controlplane.Stored[controlplane.CreatorLiveShotRun]{}, fmt.Errorf("confirm creator live-shot plan: %w", err)
		}
		...
		return controlplane.Stored[controlplane.CreatorLiveShotRun]{Value: run}, nil
	})
}
...
func (p *Postgres) completeCreatorProviderJob(ctx context.Context, runID uuid.UUID, input orchestration.ExecuteProviderJobInput, result orchestration.ProviderResult) (bool, error) {
	...
	var planHash, requestHash, upstreamTaskID, requestID string
	var route providercontract.ModelSnapshot
	var createdAt time.Time
	if err := p.pool.QueryRow(ctx, `SELECT r.plan_id,r.provider_job_id,r.reservation_id,r.reserved_video_tokens,p.plan_hash,r.request_hash,p.route_snapshot,COALESCE(r.upstream_task_id,$2),COALESCE(r.upstream_request_id,$3),r.created_at FROM video_pipeline.creator_live_shot_runs r JOIN video_pipeline.creator_live_shot_plans p ON p.id=r.plan_id WHERE r.id=$1`, runID, result.UpstreamTaskID, result.RequestID).Scan(&planID, &providerJobID, &reservationID, &reservedVideoTokens, &planHash, &requestHash, &route, &upstreamTaskID, &requestID, &createdAt); err != nil {
		return true, err
	}
	...
	_, err = p.pool.Exec(ctx, `UPDATE video_pipeline.creator_live_shot_runs SET state='SUCCEEDED',progress=100,submit_count=1,upstream_task_id=NULLIF($2,''),upstream_request_id=NULLIF($3,''),output_hash=$4,output_media_type=$5,output_size_bytes=$6,output_width=$7,output_height=$8,output_duration_ms=$9,usage=$10,cash_cost=$11,manifest=$12,manifest_hash=$13,error_code=NULL,updated_at=now(),terminal_at=now() WHERE id=$1 AND state NOT IN ('FAILED','CANCELLED')`, runID, upstreamTaskID, requestID, result.ArtifactDigest, result.MediaType, result.ArtifactSize, result.Width, result.Height, result.DurationMillis, result.Usage, manifest.CashCost, manifest, manifestHash)
	return true, err
}
...
func (p *Postgres) cancelCreatorProviderJob(ctx context.Context, runID uuid.UUID, result orchestration.CancelProviderResult) (bool, error) {
	...
	_, err := p.pool.Exec(ctx, `
		WITH changed AS (
			UPDATE video_pipeline.creator_live_shot_runs SET state='CANCELLED',updated_at=now(),terminal_at=now()
			WHERE id=$1 AND state NOT IN ('SUCCEEDED','FAILED') RETURNING operation_id
		)
		UPDATE video_pipeline.operation_requests SET state='CANCELLED',updated_at=now()
		WHERE id IN (SELECT operation_id FROM changed) AND state IN ('ACCEPTED','RUNNING','CANCEL_REQUESTED')`, runID)
	return true, err
}
