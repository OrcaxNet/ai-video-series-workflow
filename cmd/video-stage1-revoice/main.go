// Command video-stage1-revoice materializes one approved VOICE/config revision
// and emits a new prompt-free Stage 1 execution package. It has no Provider
// Adapter URL and cannot submit TTS or video jobs.
package main
...
func main() {
	if err := run(context.Background(), os.Args[1:], os.Stderr, os.LookupEnv); err != nil {
		log.Fatalf("Stage 1 revoice failed: %v", err)
	}
}
...
func run(
	...
	flags := flag.NewFlagSet("video-stage1-revoice", flag.ContinueOnError)
	flags.SetOutput(stderr)
	var planPath, parentPath, revisionPath, outputPath, reportPath string
	...
	flags.StringVar(&revisionPath, "revision", "", "approved speech VOICE revision input")
	flags.StringVar(&outputPath, "output", "", "revised execution package output")
	flags.StringVar(&reportPath, "report", "", "no-cost speech canary report output")
	...
	if flags.NArg() != 0 || planPath == "" || parentPath == "" || revisionPath == "" ||
		outputPath == "" || reportPath == "" {
		return errors.New("plan, parent, revision, output, and report are required")
	}
	...
	var revision stage1materialize.SpeechVoiceRevisionInput
	if err := decodeFile(revisionPath, &revision); err != nil {
		return fmt.Errorf("read speech VOICE revision: %w", err)
	}
	...
	revised, report, err := stage1materialize.RevoiceStage1(
		ctx, pool, cas, plan, parent, revision, stage1materialize.Approval{
			CommentID: approvalComment, ActorID: approvalActor,
	...
	if err := writeJSONAtomically(reportPath, report); err != nil {
		return fmt.Errorf("write revoice report: %w", err)
	}
	return nil
}
...
func writeJSONAtomically(path string, value any) error {
	...
	temporary, err := os.CreateTemp(directory, ".stage1-revoice-*")
	if err != nil {
		return err
	}
