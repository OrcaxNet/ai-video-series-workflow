...
# This regression specifically exercises the v7 guard. Migration v8 permits
# rollback only before any live activation, so a clean smoke database can move
# to v7 for the probe and is restored to the latest version afterwards.
run_migrate down 1 >/dev/null
...
echo "v7 protected rollback dirty-state recovery passed; migration v8 restored"
