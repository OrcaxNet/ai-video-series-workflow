...
CREATE TABLE creator_live_shot_runs (
    ...
    CHECK ((state = 'SUCCEEDED' AND output_hash IS NOT NULL AND manifest IS NOT NULL AND manifest_hash IS NOT NULL)
        OR state <> 'SUCCEEDED'),
    CHECK ((state IN ('SUCCEEDED','FAILED','CANCELLED') AND terminal_at IS NOT NULL)
...
CREATE FUNCTION guard_creator_live_shot_terminal_update()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    IF OLD.state IN ('SUCCEEDED','FAILED','CANCELLED') AND NEW IS DISTINCT FROM OLD THEN
        RAISE EXCEPTION 'creator live-shot terminal evidence is immutable';
    END IF;
    RETURN NEW;
END;
$$;

CREATE TRIGGER creator_live_shot_run_terminal_update
    BEFORE UPDATE ON creator_live_shot_runs
    FOR EACH ROW EXECUTE FUNCTION guard_creator_live_shot_terminal_update();
